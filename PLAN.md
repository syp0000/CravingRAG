...
| Near-duplicate flooding at 20k: *"warm spicy soup"* returns Hot And Sour Soup, Hot And Sour Chicken Soup, and Hot & Sour Soup as three of the five (2026-08-26). The UI dedupe is exact normalized title only; RecipeNLG has many variants of one dish. Fix is a dish-identity dedupe (title token overlap or profile-vector cosine > ~0.97 between candidates) with the dropped rows recorded as `duplicate_dish` in the decision record | Only visible at scale (the 342 corpus had one of each). Cosmetic for the consumer, but it also means the "supply" counts in the demand mart overcount distinct dishes by an unmeasured factor. |
